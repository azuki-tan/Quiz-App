    ...
    try {
        ...
        app.post('/api/exams/lookup', async (req, res) => {
            try {
                ...
                const candidateSessions = allSessions.filter(s => s.quizTargetId === exam.quizTargetId &&
                    s.learningMode === 'exam' &&
                    s.isCompleted &&
        ...
        app.post('/api/auth/exam-login', async (req, res) => {
            try {
                ...
                const examSessions = sessions.filter(s => s.quizTargetId === exam.quizTargetId &&
        ...
        app.get('/api/subjects/:subjectId/quizzes', requireAuth, async (req, res) => {
            try {
                ...
                if (!user || !user.isAdmin) {
                    list = list.filter((q) => q.isExamOnly !== 1);
                }
                res.json(list);
            }
            catch (err) {
...
Do not include any markdown backticks (like \`\`\`json) in your response, return ONLY the raw JSON text.`;
        ...
        app.get('/api/sessions/:idOrToken', async (req, res) => {
            try {
                ...
                if (exam && isScheduled) {
                    session.examEnd = exam.timeEnd;
                    session.durationTime = exam.durationTime;
                    session.isScheduledExam = true;
                    session.allowReview = exam.allowReview;
                    session.showScore = exam.showScore;
                }
                else {
                    session.isScheduledExam = false;
                }
                ...
                const prepareSessionResponse = (s) => {
                    if (!s)
                        return s;
                    if (maskScore) {
                        return {
        ...
        async function callAI(prompt, customUrl, customKey, customModel) {
        ...
        app.post('/api/ai/analyze-progress', requireAuth, async (req, res) => {
            try {
                ...
                const sessions = await db.userDb.all(`SELECT id, totalCorrect, totalWrong FROM sessions 
           WHERE quizTargetId IN (${quizPlaceholders}) AND userEmail = ?`, [...quizIds, user.email]);
                ...
                const wrongDetails = await db.userDb.all(`SELECT DISTINCT questionTargetId FROM session_details 
           WHERE learningSessionId IN (${sessionPlaceholders}) AND isCorrect = 0`, sessionIds);
                let totalCorrect = 0;
                let totalWrong = 0;
                sessions.forEach((s) => {
                ...
                const limitWrongIds = wrongDetails.slice(0, 15).map((d) => d.questionTargetId);
                const wrongPlaceholders = limitWrongIds.map(() => '?').join(',');
                // Fetch question texts
                const questions = await db.quizDb.all(`SELECT id, content, explanation FROM questions WHERE id IN (${wrongPlaceholders})`, limitWrongIds);
                // Fetch answers options for these questions
                const answers = await db.quizDb.all(`SELECT id, content, isCorrect, questionTargetId FROM answers WHERE questionTargetId IN (${wrongPlaceholders})`, limitWrongIds);
                // Construct incorrect questions text for the AI prompt
                let incorrectQuestionsText = '';
                questions.forEach((q, index) => {
                    const qAnswers = answers.filter((a) => a.questionTargetId === q.id);
...
Lưu ý: Viết báo cáo thật cuốn hút, trực quan, sử dụng các ký hiệu emoji phù hợp và định dạng Markdown để hiển thị đẹp mắt.`;
        ...
        app.get('/api/ai/analyze-repetition/:subjectId', requireAuth, async (req, res) => {
            try {
                ...
                const questions = await db.quizDb.all(`SELECT q.id, q.content, q.quizTargetId, quiz.name as quizName 
           FROM questions q
           JOIN quizzes quiz ON q.quizTargetId = quiz.id
           WHERE q.quizTargetId IN (${quizPlaceholders})`, quizIds);
                if (questions.length === 0) {
                ...
                const cleanStringForSimilarity = (str) => {
                    return str
                        .toLowerCase()
                ...
                const getWordOverlapSimilarity = (str1, str2) => {
                ...
                topDuplicatesSamples.forEach((d, idx) => {
...
Lưu ý: Trình bày đẹp mắt với các tiêu đề rõ ràng.`;
        ...
        app.post('/api/ai/recommend-questions', requireAuth, async (req, res) => {
            try {
                ...
                const quizPlaceholders = quizIds.map(() => '?').join(',');
                const allQuestions = await db.quizDb.all(`SELECT id, content FROM questions WHERE quizTargetId IN (${quizPlaceholders})`, quizIds);
                if (allQuestions.length === 0) {
                    res.status(400).json({ error: 'Không tìm thấy câu hỏi nào trong môn học này.' });
                    return;
                }
                // Fetch incorrect questions answered by the user
                const sessions = await db.userDb.all(`SELECT id FROM sessions WHERE quizTargetId IN (${quizPlaceholders}) AND userEmail = ?`, [...quizIds, user.email]);
                ...
                if (sessions.length > 0) {
                    ...
                    const wrongDetails = await db.userDb.all(`SELECT DISTINCT questionTargetId FROM session_details 
             WHERE learningSessionId IN (${sessionPlaceholders}) AND isCorrect = 0`, sessionIds);
                    incorrectQuestionIds = wrongDetails.map((d) => d.questionTargetId);
                }
                let recommendedIds = [];
                if (incorrectQuestionIds.length > 0) {
                    try {
                        // Ask AI to pick the best 10 questions targeting weak spots
                        const wrongQuestions = await db.quizDb.all(`SELECT id, content FROM questions WHERE id IN (${incorrectQuestionIds.slice(0, 10).map(() => '?').join(',')})`, incorrectQuestionIds.slice(0, 10));
                        let incorrectText = '';
...
Chỉ trả về kết quả là một mảng JSON chứa các ID câu hỏi được lựa chọn (các số nguyên), ví dụ: [12, 14, 55, 66]. KHÔNG viết thêm bất kỳ lời bình luận hay giải thích nào, chỉ trả về đúng định dạng JSON mảng.`;
                        ...
                        if (Array.isArray(parsed)) {
                            recommendedIds = parsed.filter(id => allQuestions.some((q) => q.id === id));
                        }
                    }
                    catch (e) {
                ...
                if (recommendedIds.length === 0) {
                    ...
                    const incorrectInSubject = allQuestions.filter((q) => incorrectQuestionIds.includes(q.id));
                    const remainingCount = 10 - incorrectInSubject.length;
                    let selected = [...incorrectInSubject];
                    if (remainingCount > 0) {
                        const others = allQuestions.filter((q) => !incorrectQuestionIds.includes(q.id));
                        const shuffledOthers = others.sort(() => Math.random() - 0.5).slice(0, remainingCount);
                        selected = [...selected, ...shuffledOthers];
                    }
                    recommendedIds = selected.map((q) => q.id);
                }
